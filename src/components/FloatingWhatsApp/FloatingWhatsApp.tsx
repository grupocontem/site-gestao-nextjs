...
export default function FloatingWhatsApp({
    phone = "552140403737",
    message = "",
    label = "Contato rápido via WhatsApp",
    className = "",
}: Props) {
    const href = `https://api.whatsapp.com/send?phone=${encodeURIComponent(
        phone
    ...
    return (
        <a
            href={href}
            className={`gc-whats-float ${className}`}
            target="_blank"
            rel="noopener noreferrer"
            aria-label={label}
        >
            {/* ícone svg (sem depender de lib) */}
            <svg viewBox="0 0 48 48" aria-hidden="true" focusable="false" className="gc-whats-icon">
